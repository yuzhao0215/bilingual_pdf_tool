...
FONTNAME = "CN"  # internal name in the PDF
...
def main():
    ...
    # Register/embed the font on every page (required in your PyMuPDF build)
    for page in doc:
        page.insert_font(fontname=FONTNAME, fontfile=FONTFILE)

    # Sanity text (should never be ???? if font embedding works)
    doc[0].insert_textbox(
        fitz.Rect(36, 36, 300, 80),
        "中文测试",
        fontname=FONTNAME,
        fontsize=16,
        color=(1, 0, 0),
    ...
    print("Sanity text added: 中文测试 (red). If this is ???, FONTFILE is wrong.")
    ...
    # 2) translate batches + print debug
    todo = sorted(todo)
    for i in range(0, len(todo), BATCH_SIZE):
        chunk = todo[i:i+BATCH_SIZE]
        mapping = translate_batch(client, chunk)

        print("\n=== TRANSLATION DEBUG (first 20) ===")
    ...
    # 3) insert Chinese next to each bbox
    inserted = 0
    for page, spans in zip(doc, spans_per_page):
        ...
        for src, (x0, y0, x1, y1), fs in spans:
            ...
            page.insert_textbox(
                rect,
                zh,
                fontname=FONTNAME,   # use embedded font by name
                fontsize=zh_fs,
                color=(0, 0, 0),
                overlay=True,
