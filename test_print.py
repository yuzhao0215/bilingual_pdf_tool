...
"""
    ...
    if not content:
        raise RuntimeError("Empty model output. Check API quota/billing/model.")
...
def main():
    ...
    # --- FONT SANITY TEST ---
    # Put "中文测试" at top-left of page 1 using the same fontfile.
    # If this appears as ???? then the font isn't being used.
    page0 = doc[0]
    test_rect = fitz.Rect(36, 36, 300, 80)
    ...
    print("Wrote font sanity text: 中文测试 (red). If it shows ???? your fontfile isn't being applied.")

    # 1) collect spans + unique todo
    spans_per_page = []
    todo = set()
    ...
    for i in range(0, len(todo), BATCH_SIZE):
        ...
        # DEBUG PRINT
        print("\n=== TRANSLATION DEBUG (first 20 in this batch) ===")
        for j, (k, v) in enumerate(mapping.items()):
            if j >= 20:
                break
            print(f"SRC: {k}")
            print(f"ZH : {v}")
    ...
    for page, spans in zip(doc, spans_per_page):
        ...
        for src, (x0, y0, x1, y1), fs in spans:
            ...
            page.insert_textbox(
                rect,
                zh,
                fontfile=FONTFILE,   # <-- key: force the CJK font file here
                fontsize=zh_fs,
                color=(0, 0, 0),
                overlay=True,
